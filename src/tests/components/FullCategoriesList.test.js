...
it('render FullCategoriesList correctly', () => {
  const { getByTestId } = render(
    <BrowserRouter>
      <FullCategoriesList list={list} />
    </BrowserRouter>,
  );
  expect(getByTestId('fullcategorieslist-text')).toHaveTextContent(
    /^Categories$/,
  );
});

it('render FullCategoriesList Beef correctly', () => {
  ...
  expect(getByTestId('cards-container')).toContainElement(child);
});

it('render FullCategoriesList Vegetarian correctly', () => {
  const { getByTestId, getAllByTestId } = render(
    <BrowserRouter>
      <FullCategoriesList list={list} />
    </BrowserRouter>,
  );
  const AllAray = getAllByTestId('single-index-card');
  const child = AllAray[AllAray.length - 1];
  expect(getByTestId('cards-container')).toContainElement(child);
});
